...
![Xcode indent settings](screens/line-width.png)
...
Use implicitly unwrapped types declared with `!` only for instance variables that you know will be initialized later before use, such as subviews that will be set up in `viewDidLoad`. Another example is circtular dependency resolution when implementing Dependency Injection. Generally, implicitly unwrapped types should be avoided, unless they are really nessesary. 
